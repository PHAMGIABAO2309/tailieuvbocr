use actix_web::{get, web, Responder, HttpResponse};
use sqlx::{MySql, Pool};
use serde::Deserialize;

use crate::api::tomtatvanban::db_tomtat::SQL_TOM_TAT;
use crate::api::tomtatvanban::model_tomtat::{TomTat, convert_json};

#[derive(Deserialize)]
struct QueryParams {
    file_code: Option<String>,
}

#[get("/api/tomtat")]
pub async fn api_json_tomtat
(
    db_pool: web::Data<Pool<MySql>>,
    truyvan: web::Query<QueryParams>,
) -> impl Responder 
{
    
    let file_code = truyvan.file_code.as_deref().unwrap_or("HS01");
  
    let sql = sqlx::query_as::<_, TomTat>(SQL_TOM_TAT).bind(file_code).fetch_all(db_pool.get_ref()).await;
    
    match sql 
    {
        Ok(save) => 
        {
            let kq_json = convert_json(save); 
            HttpResponse::Ok().json(kq_json)
        }
        Err(e) => HttpResponse::InternalServerError().body(format!("Lỗi truy vấn database: {}", e)),
    }
}
